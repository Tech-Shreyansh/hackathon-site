...
export default function Accordion({ question, answer }: Props) {
  ...
  return (
    <div className="w-11/12 md:w-1/2 mx-auto rounded-md border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
      <button
        role="button"
        aria-expanded={showContent}
        style={{ borderBottom: showContent ? 'solid 2px' : '0px' }}
        className="flex w-full items-center justify-between rounded-[5px] border-black bg-violet-400 p-5 font-bold"
        onClick={() => {
      ...
      >
        <p className="p-5 text-left">{answer}</p>
      </div>
    </div>
  )
}